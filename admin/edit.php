...
if ($id <= 0) {
    $_SESSION['flash'] = ['type' => 'error', 'message' => 'ID gejala tidak valid.'];
    header('Location: /admin/index.php');
    exit();
}

// Ambil data awal
$stmt = $conn->prepare('SELECT g.id, g.question_text, k.nama_kategori AS question_type, g.sort_order FROM gejala g JOIN kategori_gejala k ON g.kategori_id = k.id WHERE g.id = ? LIMIT 1');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($res_kat) {
            ...
            $stmt = $conn->prepare('UPDATE gejala SET question_text = ?, kategori_id = ?, sort_order = ? WHERE id = ?');
            $stmt->bind_param('siii', $old['question_text'], $kategori_id, $sortOrder, $id);
            if ($stmt->execute()) {
                $stmt->close();
                $_SESSION['flash'] = ['type' => 'success', 'message' => 'Gejala berhasil diperbarui.'];
                header('Location: /admin/index.php');
                exit();
            } else {
...
<head>
    ...
    <title>Edit Gejala - Admin MentalHealth</title>
...
<main class="max-w-3xl mx-auto px-4 md:px-8 py-8">
    <div class="mb-8">
        <h1 class="font-display text-3xl font-bold text-slate-800">Edit Gejala <span class="text-slate-400">#<?php echo (int)$id; ?></span></h1>
        <p class="text-slate-500 mt-1">Perbarui pertanyaan untuk skrining DASS-21.</p>
    </div>
    ...
    <form method="POST" action="/admin/edit.php?id=<?php echo (int)$id; ?>" class="bg-white rounded-2xl border border-slate-200 p-6 md:p-8 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 mt-1">Urutan tampil pertanyaan (1 = pertama).</p>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
            ...
            <button type="submit" class="inline-flex items-center gap-2 bg-sky-600 hover:bg-sky-700 text-white font-semibold rounded-full px-6 py-2.5 shadow-lg shadow-sky-200">
